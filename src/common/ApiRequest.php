...
class ApiRequest
{
    ...
    /**
     * @title get请求（新版服务端API）
     *
     * @param string $uri
     * @param array $query
     * @return mixed
     */
    public static function get_v1(string $uri, array $query = [], bool $has_token = true)
    {
        // 请求参数不为空
        if(!empty($query)){
            // 拼接url参数
     ...
     * @return mixed
     */
    public static function post_v1(string $uri, array $json = [], bool $has_token = true)
    {
        // 发起post请求
        return self::http_request_v1('post', $uri, $json, $has_token);
    }
     ...
     * @return mixed
     */
    public static function http_request_v1(string $method, string $uri, array $body = [], bool $has_token = true)
    {

        // 拼接url
        $uri = Config::getApi()['api_domain'] . $uri;
        // 基础header头信息
        $header[] = 'Content-Type: application/json';
        // 是否携带token
        if ($has_token) {
